...
const WEBAPP_URL = "https://princ3ss-fi0na.github.io/tech1/";
const ADMIN_ID = 1696302243;

const userGarages = new Map();
...
async function handleMycar(chatId) {
  const garage = userGarages.get(chatId) || [];
  ...
  if (garage.length > 0) {
    ...
    buttons.push([{ text: "🛒 Подобрать запчасти", web_app: { url: WEBAPP_URL } }]);
  }
...
async function handleEngineSelect(chatId, messageId, engine) {
  ...
  const garage = userGarages.get(chatId) || [];
  garage.forEach((c) => (c.isPrimary = false));
  garage.push(car);
  userGarages.set(chatId, garage);
  ...
  const kb = {
    reply_markup: {
      inline_keyboard: [
        [{ text: "🛒 Подобрать запчасти", web_app: { url: `${WEBAPP_URL}?car=${state.brandId}` } }],
...
async function handleOrders(chatId) {
  ...
  await sendMessage(chatId, text, {
    reply_markup: {
      inline_keyboard: [
        [{ text: "📦 Мои заказы", web_app: { url: WEBAPP_URL } }],
      ],
    },
  });
}
...
async function handleSetField(chatId, field, value) {
  ...
  if (field === "city") {
    ...
    if (hint) {
      const profile2 = userProfiles.get(chatId) || {};
      profile2[field] = value.trim();
      userProfiles.set(chatId, profile2);
...
const adminOrders = new Map(); // orderId -> order object
let adminMarkup = 0;
...
async function handleAdmin(chatId) {
  ...
  const ordersCount = adminOrders.size;
  const usersCount = userProfiles.size;
  const garagesCount = userGarages.size;
  const pendingOrders = [...adminOrders.values()].filter(o => o.status === "pending").length;
...
async function handleAdminOrders(chatId, messageId) {
  ...
  const orders = [...adminOrders.values()].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  ...
  const lines = orders.slice(0, 15).map(o => {
    ...
    const date = new Date(o.createdAt).toLocaleDateString("ru");
    const items = o.items?.map(i => i.name).join(", ") || "—";
    return `${emoji} <b>#${o.id}</b> — ${STATUS_LABEL[o.status] || o.status}\n` +
      `   ${o.customerName || "?"} · ${o.total?.toLocaleString() || 0} ₽\n` +
      `   ${o.customerRegion ? o.customerRegion + ", " : ""}${o.customerCity || ""}\n` +
      `   📅 ${date}${o.trackingNumber ? ` · 🔗 ${o.trackingNumber}` : ""}`;
  });
...
async function handleAdminOrderDetail(chatId, messageId, orderId) {
  ...
  const order = adminOrders.get(orderId);
  if (!order) {
  ...
  const text =
    ...
    `${order.trackingNumber ? `🔗 Трек: <code>${order.trackingNumber}</code> (${order.trackingCarrier || "?"})` : "🔗 Трек: не указан"}\n` +
...
async function handleAdminSetStatus(chatId, messageId, orderId, status) {
  ...
  if (order.telegramId) {
    ...
    await sendMessage(order.telegramId, userMsg);
  }

  await answerCallback(""); // already answered above
...
async function handleAdminTrackText(chatId, text) {
  ...
  const order = adminOrders.get(state.orderId);
  if (!order) return;
  ...
  adminOrders.set(state.orderId, order);
  userStates.delete(chatId);

  if (order.telegramId) {
    await sendMessage(order.telegramId,
      `📦 <b>Заказ #${order.id}</b>\n\n` +
      `🚚 Ваш заказ отправлен!\n` +
...
async function handleAdminMarkup(chatId, args) {
  ...
  if (isNaN(val) || val < 0 || val > 500) {
    return sendMessage(chatId, `💰 Текущая наценка: <b>${adminMarkup}%</b>\n\nУстановить: <code>/amarkup 15</code> (0–500)`);
  }
  adminMarkup = val;
  return sendMessage(chatId, `✅ Наценка установлена: <b>${adminMarkup}%</b>`);
}
...
async function handleAdminBroadcast(chatId, text) {
  ...
  for (const [id] of userProfiles) allUserIds.add(id);
  for (const [id] of userGarages) allUserIds.add(id);
...
async function handleAdminUsers(chatId) {
  ...
  for (const userId of allUserIds) {
    ...
    const garage = userGarages.get(userId) || [];
    ...
    lines.push(
      `👤 <b>${name}</b> (ID: <code>${userId}</code>)\n` +
      `   📱 ${phone}${region ? ` · 🗺 ${region}` : ""}${city ? ` · 🏙 ${city}` : ""}\n` +
      `   🚗 ${cars}`
    );
  }

  const text = `👥 <b>Пользователи (${allUserIds.size})</b>\n\n${lines.slice(0, 20).join("\n\n")}` +
    (lines.length > 20 ? `\n\n<i>...ещё ${lines.length - 20}</i>` : "");

  return sendMessage(chatId, text, makeKeyboard([[{ text: "← Админ", callback_data: "adm_back" }]]));
}

// Register incoming orders from WebApp notifications
function registerOrder(order) {
  if (order && order.id) {
    adminOrders.set(order.id, { ...order });
  }
}
...
function tryParseOrderFromMessage(text) {
  ...
  const id = idMatch[1];
  if (adminOrders.has(id)) return null;
